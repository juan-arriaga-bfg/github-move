...
public class UnityMessageHandler : MonoBehaviour
{
	...
	void Start ()
	{
		...
		#elif UNITY_ANDROID
			ajc = new AndroidJavaClass ("com.bigfishgames.bfgunityandroid.NotificationQueueRunner");
			
			if (ajc != null)
				ajc.CallStatic("setUnityMessageHandlerObjectName", this.gameObject.name);
		#endif
	}
	...
	void Update ()
	{
		...
		__BfgUtilities__flushNotificationQueue();
			...
			if (ajc != null)
			{
				ajc.CallStatic<bool>("FlushNotificationQueue");
			}
			#endif
		}
		--_frames_remaining;
	...
	{
		NotificationCenter.Instance.HandleNativeMessage (message);
	}
}
